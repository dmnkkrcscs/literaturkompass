...
export function SearchFilters({
  ...
  const updateFilter = useCallback((patch: Partial<SearchFiltersState>) => {
    setFilters((prev) => {
  ...
  const handleGenreToggle = useCallback((genre: string) => {
    setFilters((prev) => {
      const newGenres = prev.genres.includes(genre)
        ...
        : [...prev.genres, genre]
      const next = { ...prev, genres: newGenres }
      onFilterChangeRef.current?.(next)
      return next
    })
  }, [])
  ...
  const handleStarredOnlyToggle = useCallback(() => {
    setFilters((prev) => {
      const next = { ...prev, starredOnly: !prev.starredOnly }
      onFilterChangeRef.current?.(next)
      return next
    })
  }, [])
  ...
  const resetFilters = useCallback(() => {
    ...
    onFilterChangeRef.current?.(DEFAULT_FILTERS)
  }, [])
            ...
            <div className="flex flex-wrap gap-2">
              {['all', 'WETTBEWERB', 'ANTHOLOGIE', 'ZEITSCHRIFT'].map((type) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => updateFilter({ type: type as SearchFiltersState['type'] })}
                >
            ...
            <div className="flex flex-wrap gap-2">
              {['all', '7days', '30days', '90days'].map((range) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => updateFilter({ deadlineRange: range as SearchFiltersState['deadlineRange'] })}
                >
                  {range === 'all'
                    ? 'Alle'
                    : range === '7days'
            ...
            <div className="flex flex-wrap gap-2">
              {['deadline', 'newest', 'relevance'].map((sort) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => updateFilter({ sortBy: sort as SearchFiltersState['sortBy'] })}
                >
                  {sort === 'deadline'
                    ? 'Frist'
                    : sort === 'newest'
